...
def grab_from_fnac(url, isbn, driver):
    ...
    if len(cites)>0:
        ...
        if len(url)>0:
            ...
            soup = BeautifulSoup(driver.page_source, features="html.parser")
            time.sleep(2)
            script_text = soup.find('script', type='application/ld+json')
            if script_text is not None:
                data = json.loads(script_text.text)
                ...
                try:
                    ...
                    pass
                summary = soup.findAll("span", {"class":"js-ProductSummary-truncate"})
                if len(summary)>0:
                    text = summary[0].contents[0]
                if len(text)==0:
                    print(f'url {isbn} has no description')
            return text
    else:
...
def grab_from_bertrand(url, isbn, driver):
    ...
    for cite in cites:
        if len(cite.text)>0:
           index = cite.text.find("›")
           new_text=cite.text[index+2:]
           if new_text.find("›") != -1:
               index2 = new_text.find("›")
               output = new_text[index2+2:]
           else:
               output = cite.text[index+2:]
           break
    url = ''
