...
def grab_from_yahoo_amazon(url, isbn, driver):
...
def grab_from_yahoo_fnac(url, isbn, driver):
...
def grab_from_yahoo_franceloisir(url, isbn, driver):
    ...
    if len(cites)>0:
        if len(cites[0].attrs['href'])>0:
            driver.get(cites[0].attrs['href'])
            soup = BeautifulSoup(driver.page_source, features="html.parser")
            desc = soup.find(id="short_description_content")
            desc_p=''
            if desc is not None:
                desc_p = desc.text
            if len(desc_p)==0:
...
def grab_from_yahoo_wook(url, isbn, driver):
...
def grab_from_yahoo_leclerc(url, isbn, driver):
    search_query = 'site:culture.leclerc/ AND ' + str(isbn)
    driver.get(url)
    try:
        driver.find_element_by_name('agree').click()
    except NoSuchElementException:
    ...
    if len(cites)>0:
        if len(cites[0].attrs['href'])>0:
            driver.get(cites[0].attrs['href'])
            soup = BeautifulSoup(driver.page_source, features="html.parser")
            desc = soup.find("p",{"class":"blockdetallee"})
            desc_p=''
            if desc is not None:
                desc_p= desc.text
            if len(desc_p)==0:
...
def grab_from_yahoo_decitre(url, isbn, driver):
    ...
    if len(cites)>0:
        if len(cites[0].attrs['href'])>0:
            driver.get(cites[0].attrs['href'])
            soup = BeautifulSoup(driver.page_source, features="html.parser")
            desc = soup.find(id="resume")
            desc_p=''
            if desc is not None and desc.find("div",{"class":"content"}) is not None:
                desc_p= desc.find("div",{"class":"content"}).text
            if len(desc_p)==0:
                print(f'url {isbn} has no description')
            return desc_p
    else:
...
def grab_from_yahoo_eyrolles(url, isbn, driver):
    ...
    if len(cites)>0:
        if len(cites[0].attrs['href'])>0:
            ...
            desc = soup.find(id="resume")
            desc_p=''
            if desc is not None and desc.find("div",{"class":"reset-content"}) is not None:
                desc_p= desc.find("div",{"class":"reset-content"}).text
            if len(desc_p)==0:
                print(f'url {isbn} has no description')
            return desc_p
    else:
...
def grab_from_yahoo_placedeslibraires(url, isbn, driver):
    search_query = 'site:placedeslibraires.fr/ AND ' + str(isbn)
    driver.get(url)
    try:
        driver.find_element_by_name('agree').click()
    except NoSuchElementException:
    ...
    if len(cites)>0:
        if len(cites[0].attrs['href'])>0:
            ...
            desc_p=''
            if desc is not None and desc.find("p",{"class":"description"}) is not None:
                desc_p= desc.find("p",{"class":"description"}).text
            if len(desc_p)==0:
                print(f'url {isbn} has no description')
            return desc_p
    else:
...
def grab_from_yahoo_labaignoiredarchimede(url, isbn, driver):
    search_query = 'site:labaignoiredarchimede.com/ AND ' + str(isbn)
    driver.get(url)
    try:
        driver.find_element_by_name('agree').click()
    except NoSuchElementException:
...
def main():
    ...
    for isbn in isbn_list:
        ...
        if check_desc_already_exists(isbn) is False:
            description = grab_from_yahoo_labaignoiredarchimede(url, str(isbn), driver) 
            if len(description) == 0:
                description = grab_from_yahoo_placedeslibraires(url, str(isbn), driver) 
                if len(description) == 0:
                    description = grab_from_yahoo_eyrolles(url, str(isbn), driver) 
                    if len(description) == 0:
                        description = grab_from_yahoo_decitre(url, str(isbn), driver) 
                        if len(description) == 0:
                            description = grab_from_yahoo_franceloisir(url, str(isbn), driver) 
                            if len(description) == 0:
                                description = grab_from_yahoo_leclerc(url, str(isbn), driver) 
                                if len(description) == 0:
                                    description = grab_from_yahoo_wook(url, str(isbn), driver) 
                                    if len(description) == 0:
