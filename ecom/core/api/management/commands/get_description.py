...
def grab_from_leclerc(url, isbn, driver):
...
def grab_from_placedeslibraires(url, isbn, driver):
...
def grab_from_labaignoiredarchimede(url, isbn, driver):
...
def grab_from_ombresblanches(url, isbn, driver):
    ...
    if len(cites)>0:
        if len(cites[0].attrs['href'])>0:
            driver.get(cites[0].attrs['href'])
            soup = BeautifulSoup(driver.page_source, features="html.parser")
            desc = soup.find("div", {"class":"resume"})
            desc_p=''
            if desc is not None and desc.find("p") is not None:
                desc_p= desc.find("p").text
            if len(desc_p)==0:
...
def grab_from_amazoncom(url, isbn, driver):
    ...
    if len(cites)>0:
        if len(cites[0].attrs['href'])>0:
            driver.get(cites[0].attrs['href'])
            soup = BeautifulSoup(driver.page_source, features="html.parser")
            desc = soup.find(id="productDescription_fullView")
            desc_p=''
            if desc is not None and desc.findAll("p") is not None:
                for x in desc.findAll("p"):
                    if len(x.text)>10:
                        desc_p= x.text
                        break
    ...
    for isbn in isbn_list:
        ...
        if check_desc_already_exists(isbn) is False:
            ...
            if len(description) == 0:
                description = grab_from_ombresblanches(url, str(isbn), driver) 
                if len(description) == 0:
                    description = grab_from_labaignoiredarchimede(url, str(isbn), driver) 
                    if len(description) == 0:
                        description = grab_from_placedeslibraires(url, str(isbn), driver) 
                        if len(description) == 0:
                            ...
                            if len(description) == 0:
                                ...
                                if len(description) == 0:
                                    ...
                                    if len(description) == 0:
                                        description = grab_from_leclerc(url, str(isbn), driver) 
                                        if len(description) == 0:
