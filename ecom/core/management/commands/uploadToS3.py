...
def upload_to_aws(local_file, bucket, s3_file):
    s3 = boto3.client('s3', aws_access_key_id=os.environ.get(
        'S3_ACCESS_KEY'), aws_secret_access_key=os.environ.get('S3_SECRET_KEY'))
...
def connect_bucket(name):
    try:
        s3resource = boto3.resource('s3', aws_access_key_id=os.environ.get(
            'S3_ACCESS_KEY'), aws_secret_access_key=os.environ.get('S3_SECRET_KEY'))
...
def google_it(placeholders, conn):
    ...
    # main download
    arguments = {"keywords": str(isbn), "limit": 1, "print_urls": True, "format": "jpg",
                 "size": '>400*300', "output_directory": placeholders}  # creating list of arguments
...
def store_it(bucket_name, placeholders):
    ...
    if bucket == 'does not exist':
        s3_resource = boto3.resource('s3', aws_access_key_id=os.environ.get(
            'S3_ACCESS_KEY'), aws_secret_access_key=os.environ.get('S3_SECRET_KEY'))
